...
export class GameManager {
    ...
    async getCharacter(userId) {
        ...
        if (data && data.state) {
            ...
            if (data.current_activity && data.activity_started_at) {
                const now = new Date();
                const lastTick = data.last_saved ? new Date(data.last_saved) : new Date(data.activity_started_at);
                ...
                if (elapsedSeconds >= timePerAction) {
                    ...
                    if (actionsToProcess > 0) {
                        ...
                        await this.processBatchActions(data, actionsToProcess);
                        updated = true;
    ...
    async processBatchActions(char, quantity) {
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            if (result && !result.error) {
                ...
                break; // Parar se acabar ingrediente
            }
        }
    ...
    async processGathering(char, item) {
        ...
        this.addItemToInventory(char, item.id, 1);

        const skillKey = this.getSkillKeyForResource(item.id);
        const leveledUp = this.addXP(char, skillKey, item.xp || 5);

        return { success: true, message: `Coletou ${item.name}`, leveledUp };
    }
    ...
    async processRefining(char, item) {
        ...
        const leveledUp = this.addXP(char, skillKey, item.xp || 10);

        return { success: true, message: `Refinou ${item.name}`, leveledUp };
    }
    ...
    async processCrafting(char, item) {
        ...
        this.addItemToInventory(char, finalItemId, 1);

        const skillKey = this.getSkillKeyForCrafting(item.id);
        const leveledUp = this.addXP(char, skillKey, item.xp || 50);

        return { success: true, message: `Fabricou ${qualityName}${item.name}`, leveledUp };
    }
