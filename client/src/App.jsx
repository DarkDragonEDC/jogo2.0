...
function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'market':
        return <MarketPanel socket={socket} gameState={gameState} silver={gameState.state?.silver || 0} onShowInfo={setInfoItem} />;
      case 'gathering':
      case 'refining': {
      ...
      case 'inventory':
        return <InventoryPanel inventory={gameState.state?.inventory || {}} socket={socket} silver={gameState.state?.silver || 0} onShowInfo={setInfoItem} />;
      case 'ranking':
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0a0a0f', color: '#fff', fontFamily: "'Inter', sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      <ChatWidget socket={socket} user={session.user} characterName={gameState.name} />
      <ActivityWidget gameState={gameState} onClaim={claimReward} />
